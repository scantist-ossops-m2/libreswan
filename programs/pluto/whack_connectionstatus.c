...
#undef COMBO
	...
	SHOW_JAMBUF(s, buf) {
		...
		jam(buf, " dont_frag: %s;", bool_str(c->config->child_sa.iptfs_dont_frag));
		jam(buf, " pkt_size: %ju;", c->config->child_sa.iptfs_pkt_size);
		jam(buf, " max-queue-size: %ju", c->config->child_sa.iptfs_max_qsize);
		jam(buf, " drop-time: %ju", c->config->child_sa.iptfs_drop_time);
		jam(buf, " in-delay: %ju", c->config->child_sa.iptfs_in_delay);
